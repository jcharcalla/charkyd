...
do
	...
	then
		...
		SERVICESTATUS_STRING=$(${ETCDCTL_BIN} --endpoints=${ETCD_ENDPOINTS} get --prefix ${PREFIX_STATUS}/${REGION}/${RACK}/${SERVICE_NODEID}/${SERVICENAME})
	        SERVICESTATUS_EPOCH=$(echo ${SERVICESTATUS_STRING} | grep nodeid | cut -d "," -f5 | cut -d ":" -f2)

		if [ ${SERVICESTATUS_EPOCH} -le $((`date +%s` - ${MONITOR_TIMEOUT})) ]
                then
			...
			# evict the node if its down.
                	...
                	SERVICENODE_EPOCH=$(echo ${SERVICENODE_STRING} | grep nodeid | cut -d "," -f8 | cut -d ":" -f2)

                	if [ ${SERVICENODE_EPOCH} -le $((`date +%s` - ${MONITOR_TIMEOUT})) ]
                	then
                        	# Remove the monitor services from possibly dead node
                        	# if not, evict it and elect a new one
                        	${ETCDCTL_BIN} --endpoints=${ETCD_ENDPOINTS} del --prefix ${PREFIX_MONITOR}/${REGION}/${RACK}/${SERVICE_NODEID}
		...
		# else the service is fine and we should report on it
		else
		EPOCH=$(date +%s)
		# Inform the node running the service that we are watching it elsewhere in the cluster
		#${ETCDCTL_BIN} --endpoints=${ETCD_ENDPOINTS} put ${PREFIX_MONITOR}/${REGION}/${RACK}/${SERVICE_NODEID}/${SERVICENAME}_$(openssl rand -hex ${SERVICE_UUID_LENGTH}) servicename:monitor,unit_file:na,replicas:na,nodeid:${HOSTID},epoch:${EPOCH}
		${ETCDCTL_BIN} --endpoints=${ETCD_ENDPOINTS} put ${PREFIX_MONITOR}/${REGION}/${RACK}/${SERVICE_NODEID}/${SERVICENAME}_MON${HOSTID} servicename:monitor,unit_file:na,replicas:na,nodeid:${HOSTID},epoch:${EPOCH}

		# Update the status of the service we are monitoring
		${ETCDCTL_BIN} --endpoints=${ETCD_ENDPOINTS} put ${PREFIX_MONITOR}/${REGION}/${RACK}/${HOSTID}/${SERVICENAME} servicename:${SERVICENAME},unit_file:na,replicas:na,nodeid:${SERVICE_NODEID},epoch:${EPOCH}
		fi
